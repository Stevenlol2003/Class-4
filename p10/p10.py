...
# ```
#
# and that `stars_1_rows` has **1508** rows of which the **first three** are:
#
# ```python
# [['11 Com', 'G8 III', '4742.00', '19.00', '2.70', '2.243', '2.31', ''],
#  ['11 UMi', 'K4 III', '4213.00', '29.79', '2.78', '2.430', '1.93', '1.560'],
...
stellar_effective_temperature = star_cell(2, 'Stellar Effective Temperature [K]', stars_1_rows)
stellar_radius = star_cell(2, 'Stellar Radius [Solar Radius]', stars_1_rows)
stellar_mass = star_cell(2, 'Stellar Mass [Solar mass]', stars_1_rows)
stellar_luminosity = star_cell(2, 'Stellar Luminosity [log(Solar)]', stars_1_rows)
stellar_surface_gravity = star_cell(2, 'Stellar Surface Gravity [log10(cm/s**2)]', stars_1_rows)
stellar_age = star_cell(2, 'Stellar Age [Gyr]', stars_1_rows)

third_star = Star(spectral_type, stellar_effective_temperature, stellar_radius, stellar_mass, stellar_luminosity, 
...
# |`stellar_radius`|Stellar Radius [Solar Radius]|**float**|
# |`stellar_mass`|Stellar Mass [Solar mass]|**float**|
# |`stellar_metallicity`|Stellar Metallicity [dex]|**float**|
# |`stellar_luminosity`|Stellar Luminosity [log(Solar)]|**float**|
# |`stellar_surface_gravity`|Stellar Surface Gravity [log10(cm/s**2)]|**float**|
# |`stellar_age`|Stellar Age [Gyr]|**float**|
#
...
#                 stellar_radius=19.0, stellar_mass=2.7, stellar_metallicity=-0.35, 
#                 stellar_luminosity=2.243, stellar_surface_gravity=2.31, stellar_age=None),
#  '11 UMi': Star(spectral_type='K4 III', stellar_effective_temperature=4213.0, 
#                 stellar_radius=29.79, stellar_mass=2.78, stellar_metallicity=-0.02, 
#                 stellar_luminosity=2.43, stellar_surface_gravity=1.93, stellar_age=1.56),
#  '14 And': Star(spectral_type='K0 III', stellar_effective_temperature=4813.0, 
#                 stellar_radius=11.0, stellar_mass=2.2, stellar_metallicity=-0.24, 
#                 stellar_luminosity=1.763, stellar_surface_gravity=2.63, stellar_age=4.5),
#  ...
# }
# ```
...
stars_1_dict = get_stars(stars_paths[4])
# -
...
for star_name, star_data in stars_1_dict.items():
    if star_data.stellar_luminosity != None:
        total += star_data.stellar_luminosity
        count += 1
...
stars_2_dict = get_stars(stars_paths[3])

total = 0
count = 0

for star_name, star_data in stars_2_dict.items():
    if star_data.stellar_age != None:
        total += star_data.stellar_age
        count += 1
...
for star_name, star in stars_dict.items():
    if star.stellar_radius != None and star.stellar_radius > max_radius:
...
for star_name in stars_dict:
    if star_name.startswith('Kepler'):
        ...
        if star.stellar_age != None:
            total += star.stellar_age
            count += 1

avg_age_kepler = total / count
...
# ```
#
# and that `planets_1_rows` has **1508** rows of which the **first three** are:
#
# ```python
# [['11 Com b', 'Radial Velocity', '2007', '0', '326.03', '12.1', '6165.6', '1.29', '0.231', '', ''],
#  ['11 UMi b', 'Radial Velocity', '2009', '0', '516.21997', '12.3', '4684.8142', '1.53', '0.08', '', ''],
...
mapping_1_json = read_json(file_paths[14])
...
fifth_planet = Planet(planet_name = planet_cell(4, 'Planet Name', planets_1_rows),
                      host_name = mapping_1_json[planet_cell(4, 'Planet Name', planets_1_rows)],
...
planets_1 = get_planets(csv_file_paths[9], file_paths[14])
...
planets_2 = get_planets(csv_file_paths[8], file_paths[13])
controversial_planets = [planet for planet in planets_2 if planet.controversial_flag == True]
...
planets_4 = get_planets(csv_file_paths[6], file_paths[11])
...
planets_disc_2022 = len([planet for planet in planets_list if planet.discovery_year == 2022])
...
for planet in planets_list:
    if planet.planet_radius is None:
        continue
    
    star = None
    for s in stars_dict:
        if s == planet.host_name:
            star = s
            break
    
    if star is None or stars_dict[s].stellar_radius is None or stars_dict[s].stellar_radius < 10:
        continue
    
    total+= planet.planet_radius
    count += 1


avg_planet_radius_big_stars = total / count
...
for star in stars_dict:
    if youngest_star is None or (stars_dict[star].stellar_age is not None and stars_dict[star].stellar_age < stars_dict[youngest_star].stellar_age):
        youngest_star = star
