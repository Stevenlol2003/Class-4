...
def regression_line_plot(df, x, y):
    df_fit = get_regression_line(df, x, y)
    ax = df.plot.scatter(x = x, y = y, color = 'black')
    df_fit.plot.line(x=x, y = 'fit', color = 'red', ax = ax)
    ax.set_xlabel(x)
    ax.set_ylabel(y)
...
download('QSranking.json', 'https://git.doit.wisc.edu/cdis/cs/courses/cs220/cs220-s23-projects/-/raw/main/p13/QSranking.json')
...
qs_ranking.to_sql('rankings', conn, if_exists = 'replace')
# + tags=[]
...
uw_rating = pd.read_sql("SELECT * FROM rankings WHERE institution_name = 'University Of Wisconsin-Madison' ORDER BY year ASC", conn)
uw_rating = uw_rating.set_index('index', drop = True)
...
japan_inter = pd.read_sql("SELECT institution_name, international_students FROM rankings WHERE country = 'Japan' AND year = 2020", conn)
japan_top_10_inter = japan_inter.sort_values(by = 'international_students', ascending = False).head(10)
...
all_institutions = pd.read_sql("SELECT country, COUNT(*) AS num_of_institutions FROM rankings WHERE year = 2020 GROUP BY country ORDER BY num_of_institutions DESC, country ASC", conn)
Other = pd.DataFrame([{'country': 'Other', 'num_of_institutions': all_institutions.iloc[10:]['num_of_institutions'].sum()}])
num_institutions = top_10_countries.append(Other, ignore_index = True)
...
# do NOT plot just yet
top_ranked_inter_faculty = pd.read_sql("SELECT country, international_students, faculty_student_score FROM rankings WHERE year = '2020' AND rank IN (SELECT MIN(`rank`) FROM rankings WHERE year = '2020' GROUP BY country) ORDER BY country ASC", conn)
top_ranked_inter_faculty = top_ranked_inter_faculty.drop_duplicates('country')
...
df = pd.read_sql("SELECT international_students, overall_score FROM rankings WHERE year = '2020' AND country = 'United Kingdom' AND rank <= 100", conn)
uk_inter_score_corr = abs(df['international_students'].corr(df['overall_score']))
...
df = pd.read_sql("SELECT citations_per_faculty, international_faculty, overall_score FROM rankings WHERE year = '2019'", conn)
df['citations_per_international'] = df['citations_per_faculty'] * df['international_faculty'] / 100
cit_per_inter_score_corr = abs(df['citations_per_international'].corr(df['overall_score']))
...
top_cit_per_inter = pd.read_sql("SELECT country, SUM(citations_per_international) AS sum_inter_citations FROM (SELECT country, (citations_per_faculty * international_faculty) / 100 AS citations_per_international FROM rankings WHERE year = '2019') AS temp GROUP BY country ORDER BY sum_inter_citations DESC LIMIT 15", conn)
...
avg_cit_per_inter = pd.read_sql("SELECT country, AVG(citations_per_faculty * international_faculty / 100) AS avg_inter_citations FROM rankings WHERE year = '2019' AND rank <= 300 GROUP BY country ORDER BY avg_inter_citations DESC", conn)
...
max_cit_per_inter = pd.read_sql("SELECT country, institution_name, MAX(citations_per_faculty * international_faculty / 100) AS max_inter_citations FROM rankings WHERE year = '2020' GROUP BY country HAVING max_inter_citations IS NOT NULL ORDER BY max_inter_citations DESC", conn)
...
# do NOT plot just yet
overall_rank = pd.read_sql_query("SELECT overall_score, rank FROM rankings WHERE year = '2020'", conn)
overall_rank = get_regression_line(overall_rank, 'rank', 'overall_score')
...
# do NOT plot just yet
inverse_overall_rank = pd.read_sql_query("SELECT 1/overall_score AS inverse_overall_score, rank FROM rankings WHERE year = '2020'", conn)
inverse_overall_rank = get_regression_line(inverse_overall_rank, 'rank', 'inverse_overall_score')
...
m, b = get_regression_coeff(inverse_overall_rank, 'inverse_overall_score', 'rank')
